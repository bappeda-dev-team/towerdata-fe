...
interface Modal {
    ...
    onClose: () => void;
    onSuccess?: () => void;
    existingSubKegiatanCodes?: string[];
}

type SubKegiatanDropdownOption = OptionTypeString & { namaSubKegiatan: string };

export const ModalPilihSubKegiatan: React.FC<Modal> = ({ nama_opd, isOpen, onClose, onSuccess, existingSubKegiatanCodes }) => {

    const [SubKegiatanTerpilih, setSubKegiatanTerpilih] = useState<OptionTypeString[]>([]);
    const [SubKegiatanOptions] = useState<SubKegiatanDropdownOption[]>([]);
    const { toastSuccess } = useToast();
    const isSubKegiatanLoading = false;
    const SubKegiatanError: string | null = null;
    const isSaving = false;
    const saveError: string | null = null;
    const selectionWarningMessage: string | null = null;
    const isOpdOrTahunMissing = false;
    const isSaveDisabled = SubKegiatanTerpilih.length === 0 || isSaving;

    const handleSimpan = async () => {
        if (isSaveDisabled) return;
        toastSuccess("SubKegiatan berhasil disimpan (tampilan saja)");
        setSubKegiatanTerpilih([]);
        onSuccess?.();
        onClose();
    };
        ...
        <ModalComponent isOpen={isOpen} onClose={onClose}>
            <ModalHeader className="border-sky-700">
                ...
                <div className="flex flex-wrap items-center gap-1">
                    <p className="text-sky-700">Tambah SubKegiatan di</p>
            ...
            <form action="" className="py-3 min-h-[400px]">
                <FloatingLabelSelect
                    ...
                    label="SubKegiatan"
                    isMulti
                    isLoading={isSubKegiatanLoading}
                    disable={isOpdOrTahunMissing}
                    options={SubKegiatanOptions}
                    value={SubKegiatanTerpilih}
                    onChange={(selected) => setSubKegiatanTerpilih((selected as OptionTypeString[]) || [])}
                />
