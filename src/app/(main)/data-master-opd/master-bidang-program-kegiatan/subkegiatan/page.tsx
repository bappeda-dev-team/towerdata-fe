...
import { ModalPilihSubKegiatan } from "./comp/ModalPilihSubKegiatan";
import { GetResponseSubKegiatanOpd } from "../type";
import { ErrorMessage } from "@/src/components/ui/error";

const MasterSubKegiatan = () => {

    const Jenis: "Program" | "Kegiatan" | "Sub Kegiatan" = "Sub Kegiatan";
    const [ModalOpen, setModalOpen] = useState<boolean>(false);
    const { branding } = useBrandingContext();
    const data: GetResponseSubKegiatanOpd[] = [];
    const loading = false;
    const error = false;
    const refetch = () => {};
    ...
    return (
        <>
            ...
            {ModalOpen &&
                <ModalPilihSubKegiatan
                    nama_opd={branding?.opd?.label || "-"}
                    isOpen={ModalOpen}
                    existingSubKegiatanCodes={data?.map((item) => item.kodeSubKegiatanOpd) ?? []}
