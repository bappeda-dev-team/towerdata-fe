...
import { ModalPilihProgram } from "./comp/ModalPilihProgram";
import { GetResponseProgramOpd } from "../type";
import { ErrorMessage } from "@/src/components/ui/error";

const MasterProgram = () => {

    const Jenis: "Program" | "Kegiatan" | "Sub Kegiatan" = "Program";
    const [ModalOpen, setModalOpen] = useState<boolean>(false);
    const { branding } = useBrandingContext();
    const data: GetResponseProgramOpd[] = [];
    const loading = false;
    const error = false;
    const refetch = () => {};
        ...
        <>
            ...
            {ModalOpen &&
                <ModalPilihProgram
                    ...
                    isOpen={ModalOpen}
                    existingProgramCodes={data?.map((item) => item.kodeProgramOpd) ?? []}
