...
interface Modal {
    ...
    isOpen: boolean;
    onClose: () => void;
    existingProgramCodes?: string[];
    onSuccess?: () => void;
}

type ProgramDropdownOption = OptionTypeString & { namaProgram: string };

export const ModalPilihProgram: React.FC<Modal> = ({ nama_opd, isOpen, onClose, onSuccess, existingProgramCodes }) => {

    const [ProgramTerpilih, setProgramTerpilih] = useState<OptionTypeString[]>([]);
    const [programOptions] = useState<ProgramDropdownOption[]>([]);
    const { toastSuccess } = useToast();
    const isProgramLoading = false;
    const programError: string | null = null;
    ...
    const handleSimpan = async () => {
        ...
        toastSuccess("Program berhasil disimpan (tampilan saja)");
        setProgramTerpilih([]);
        onSuccess?.();
        onClose();
    };
        ...
        <ModalComponent isOpen={isOpen} onClose={onClose}>
            <ModalHeader className="border-sky-700">
                ...
                <div className="flex flex-wrap items-center gap-1">
                    <p className="text-sky-700">Tambah Program di</p>
            ...
            <form action="" className="py-3 min-h-[400px]">
                <FloatingLabelSelect
                    ...
                    value={ProgramTerpilih}
                    onChange={(selected) => setProgramTerpilih((selected as OptionTypeString[]) || [])}
                />
