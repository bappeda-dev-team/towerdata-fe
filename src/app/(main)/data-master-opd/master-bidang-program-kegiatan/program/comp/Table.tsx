...
import { GetResponseProgramOpd } from "../../type";
...
interface TableProps {
    jenis: string;
    Data: GetResponseProgramOpd[];
    apiUrl?: string;
    onDeleteSuccess?: () => void;
}

const Table: React.FC<TableProps> = ({ jenis, Data, apiUrl, onDeleteSuccess }) => {
    const [visibleData] = useState<GetResponseProgramOpd[]>(Data);
        ...
        <TableComponent className={borderClass}>
            <table className="w-full">
                ...
                <tbody>
                    {rows.length > 0 ? (
                        rows.map((item, index) => (
                            <tr key={item.kodeProgramOpd}>
                                <td className={`px-6 py-4 border ${borderClass} text-center`}>{index + 1}</td>
                                <td className={`px-6 py-4 border ${borderClass}`}>{item.namaProgramOpd}</td>
                                <td className={`px-6 py-4 border ${borderClass}`}>{item.kodeProgramOpd}</td>
